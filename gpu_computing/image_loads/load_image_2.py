...
for file in os.listdir(path):
    if file.lower().endswith('.jpg'):   # check pour ne pas intégrer d'autres types de fichier
        file_path = path + file
        label = file[0]
...
with open('train_500Images.csv', mode='w') as csv_file:  
    csv_writer = csv.writer(csv_file, lineterminator='\n')   # lineterminator='\n' => rend le code Windows compliant
    for item in train_list:
        csv_writer.writerow([item[0], item[1]])  

with open('test_500Images.csv', mode='w') as csv_file:  
    csv_writer = csv.writer(csv_file, lineterminator='\n')   # lineterminator='\n' => rend le code Windows compliant
